...
public class AnalyzerPageTest {
  ...
  private String singleTest_populateTable_javascript = "var Obj =   {" +
      "\"builds\":[\"2\",\"1\"]," +
      "\"results\":   [{" +
      "\"buildResults\":  [{" +
      "\"buildNumber\":\"2\"," +
  ...
  public void newFailuresTest_noBuild() throws Exception {
    ...
    try {
      DomElement exclamation_mark = (DomElement) job
          .getByXPath("//*[contains(concat(' ', @class, ' '), ' icon-exclamation-sign ')]").get(0);
      assertTrue(
          "The table has an new failure exclamation mark element even though there are no builds.",
          false);
    } catch (Exception e) {
      assertTrue(true);
    }
  ...
  public void searchTest_noMatch() throws Exception {
    HtmlPage job = setupFreeStyle();
    String javaScriptCommand =
        singleTest_populateTable_javascript + "$j(\"#filter\").val(\"was\");searchTests();";
    ScriptResult result = job.executeJavaScript(javaScriptCommand);
    DomElement row = (DomElement) job.getByXPath("//*[contains(@class, 'cs427')]").get(0);
  ...
  public void searchTest_emptyFilter() throws Exception {
    HtmlPage job = setupFreeStyle();
    String javaScriptCommand =
        singleTest_populateTable_javascript + "$j(\"#filter\").val(\"\");searchTests();";
    ScriptResult result = job.executeJavaScript(javaScriptCommand);
    DomElement row = (DomElement) job.getByXPath("//*[contains(@class, 'cs427')]").get(0);
    assertTrue(row.getAttribute("style").contains("table-row"));
  }
